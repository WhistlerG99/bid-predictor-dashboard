...
def test_update_performance_history_appends_new_dates(tmp_path):
    dataset = _build_sample_dataset()
    history_path = tmp_path / "history.parquet"

    initial = history_data.compute_daily_performance_history(dataset.iloc[:2], threshold=0.5)
    initial.to_parquet(history_path, index=False)

    updated = history_data.update_performance_history_file(
        dataset, str(history_path), threshold=0.5
    )
